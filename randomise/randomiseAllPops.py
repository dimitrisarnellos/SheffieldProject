...
def memEfficient(column, first9, count):
	...
	for i in range(len(first9)):
		print(first9[i], end="")
		for j in range(451):
			gt = numpy.random.choice(["0|0", "0|1", "1|0", "1|1"],  p=[column[j][0]/count, column[j][1]/count, column[j][2]/count, column[j][3]/count])
			if j < 450:
				print(gt, end="\t")
			else:
				print(gt)
		count2 += 2
		sys.stderr.write("\r" + "Lines outputed: " + str(count2))
		sys.stderr.flush()		
