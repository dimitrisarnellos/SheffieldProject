...
def randomise_genotypes(vcf_file):
	'''
	Counts the four genotype types of each individual to have a matrix
	...
	del(data_first_row)
	# row_len - 9 for the 9 first non genotype columns and -3 for the 3 ancients
	modern_humans_number = row_len - 9 - 3
	zero_zero = 0
	zero_one = 0
	one_zero = 0
	one_one = 0
	...
	for row in data_all_rows:
		...
		ancient1.append(row[-3])
		ancient2.append(row[-2])
		ancient3.append(row[-1])
		for individual_no in range(modern_humans_number):
			...
			elif row[9 + individual_no] == "1|0":
				column[individual_no][2] += 1
			elif row[9 + individual_no] == "1|1":
				column[individual_no][3] += 1
		count += 1
	...
	for individual_no in range(modern_humans_number):
		gt = numpy.random.choice(["0|0", "0|1", "1|0", "1|1"], count, p=[column[individual_no][0]/count, column[individual_no][1]/count, column[individual_no][2]/count, column[individual_no][3]/count])
	...
	df9['c' + str(modern_humans_number)] = pd.Series(ancient1).values
	df9['c' + str(modern_humans_number + 1)] = pd.Series(ancient2).values
	df9['c' + str(modern_humans_number + 2)] = pd.Series(ancient3).values
